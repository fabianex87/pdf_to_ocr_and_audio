...
# ✅ 2️⃣ **Seleziona la Voce Corretta**
def set_voice_by_language(engine, lang):
    """
    Seleziona una voce in base alla lingua specificata.
    """
    lang_map = {
        'spa': 'spanish',
        'ita': 'italian',
...
def pdf_to_audio(pdf_path, audio_path, lang='spa'):
    try:
        ...
        # Salva il testo come file audio
        engine.save_to_file(text, audio_path)
        engine.runAndWait()
        
        print(f"✅ Audiolibro salvato con successo: {audio_path}")
    
    except Exception as e:
        print(f"❌ Errore nell'audiolibro: {e}")


# ✅ 4️⃣ **Workflow Completo: PDF → PDF OCR → Audiolibro**
...
pdf_to_ocr_and_audio('documento.pdf', 'documento_ocr.pdf', 'audiolibro.mp3', lang='spa')
